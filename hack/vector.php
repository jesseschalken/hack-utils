...
function sort<T>(array<T> $list, fun2<T, T, int> $cmp): array<T> {
  \usort($list, $cmp);
  return $list;
}

function filter<T>(array<T> $list, fun1<T, bool> $f): array<T> {
...
function map<Tin, Tout>(array<Tin> $list, fun1<Tin, Tout> $f): array<Tout> {
...
function any<T>(array<T> $a, fun1<T, bool> $f): bool {
  foreach ($a as $x) {
    if ($f($x)) {
      return true;
    }
...
function all<T>(array<T> $a, fun1<T, bool> $f): bool {
  foreach ($a as $x) {
    if (!$f($x)) {
      return false;
    }
